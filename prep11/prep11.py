...
def mergesort3(lst: List) -> List:
    ...
    if len(lst) < 2:  # We've provided the base case for you.
        ...
        n = len(lst) // 3
        sorted1 = mergesort3(lst[:n])
        sorted2 = mergesort3(lst[n:2 * n])
        sorted3 = mergesort3(lst[2 * n:])
        return merge3(sorted1, sorted2, sorted3)
...
def merge3(lst1: List, lst2: List, lst3: List) -> List:
    ...
    while index1 < len(lst1) and index2 < len(lst2):
        if lst1[index1] <= lst2[index2]:
            ...
            merged.append(lst2[index2])
            index2 += 1
    result = merged + lst1[index1:] + lst2[index2:]
    return _merge(result, lst3)
